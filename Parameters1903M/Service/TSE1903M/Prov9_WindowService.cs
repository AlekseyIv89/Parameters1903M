...
{
    class Prov9_WindowService : IWindowService
    {
        public Prov9_Window GetProvWindow() => Application.Current.Windows.OfType<Prov9_Window>().FirstOrDefault();
        ...
        public void OpenDialog(object param)
        {
            ...
            {
                Prov9_Window window = Application.Current.Windows.OfType<Prov9_Window>().FirstOrDefault();
                if (window == null)
                {
                    window = new Prov9_Window(parameter) { Owner = Application.Current.MainWindow };
                }
                window.ShowDialog();
